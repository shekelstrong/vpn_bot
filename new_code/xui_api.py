...
- Bearer Token API (3x-ui v3.1.0+) — рекомендуется, использует XUI_API_TOKEN
- Legacy Cookie API (3x-ui v2.9.x) — fallback через логин/пароль
...
class XUIService:
    ...
    def __init__(self):
        ...
        # Режим авторизации: Bearer token (v3.1.0+) или Cookie (legacy)
        self._bearer_token = settings.XUI_API_TOKEN.strip()
        if self._bearer_token:
            logger.info("3x-ui API: Bearer Token режим (v3.1.0+)")
        else:
            logger.info("3x-ui API: Legacy Cookie режим")
    ...
    async def _ensure_login(self) -> None:
        """Проверить/обновить сессию 3x-ui (только для legacy режима)."""
        # Если используем Bearer token — логин не нужен
        if self._bearer_token:
            return
        ...
        try:
            ...
            if "3x-ui=" in set_cookie:
                ...
                logger.debug("Успешная авторизация в 3x-ui (cookie)")
            else:
    ...
    async def _request(
        ...
        for attempt in range(retry):
            try:
                ...
                # Bearer 401 или Cookie 401/302 → перелогиниться
                if response.status_code in (401, 302):
                    if self._bearer_token:
                        logger.error(f"3x-ui вернул 401 — Bearer token невалиден")
                        raise Exception("3x-ui Bearer token unauthorized")
